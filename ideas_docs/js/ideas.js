var ideas = (function () {
	...
	function set_data_display_report(data) {
		...
		if ('biogrid' in data.edge_counts) {
			...
			if ('ERROR' in data.edge_counts["biogrid"]) {
                ...
                //resStr = resStr + "BioGRID error: "+data.edge_counts["biogrid"]['ERROR'];
			} else {
				var tempResStr = ""
	...
	function refresh_plots(geneArray, panel) {
		...
		var panelString = '<div style=\"margin: 0; overflow: auto; width: '+plotWidth+'px;\">';
        for(var x = 0;x<maxPlotting;x++)
        {
            var w = plotWidth;
            if( $('#plotting'+x+'size').val() == 'Half')
                w /= 2;
            panelString += '<div id=\"plottingWindow'+x+'\" style=\"float: left; width: '+w+'px;\"></div>';
        }    
	...
	function heatmapProcessing(nodes) {
		...
		var heatmapsWin=window.open('heatmaps.html', '_blank');
 		$(heatmapsWin).load(function(){
			...
			//console.log(JSON.stringify(heatmap_data,null,4));
	...
	function initialise() {
		...
		$('#heatmap_all_nodes').on("click", function() {
			...
			} else if (vis.nodes().length <= heatmap_max_nodes) {
				//Construct array of visible nodes
				heatmapProcessing(vis.nodes());
			} else {
				$('#data_div').append(" Please select a maximum of "+heatmap_max_nodes+" genes.");
			}
		});
		...
		$('#heatmap_selected_nodes').on("click", function() {
			...
			} else if (vis.selected("nodes").length >= heatmap_max_nodes) { 
				$('#data_div').append(" Please select a maximum of "+heatmap_max_nodes+" genes.");
		...
		$('#upload_exp_file').on('click', function(evt) {
			...
			if (expFiles.length > 0) {
                //do the same loading behaviour as show connections
                $("body").addClass("loading");
				...
				expFileReader.onload = function(event) {
					...
					if (!$.isEmptyObject(experData)) {
						...
						$(".plotclass").attr("disabled", false);
   						$(".plotoptions").attr("disabled", false);
					} 
                    $("body").removeClass("loading");
				}
